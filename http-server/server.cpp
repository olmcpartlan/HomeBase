...
void handle_request( beast::string_view doc_root, http::request<Body, http::basic_fields<Allocator>>&& req, Send&& send)
{
    ...
    std::cout << "Request start: " << req.target() << std::endl;
    ...
    // Calling the home dir and replace with full path.
    else 
    {
      std::cout << "CONTAINS ~" << std::endl;
      path = std::string(req.target());
      // TODO: Need to dynamically find the root path
      path = path.replace(0, 2, "/Users/owenmcpartlan");
    ...
    if(!boost::filesystem::is_directory(path.c_str()) && !req.target().find(".mp4"))
    {
    ...
    if(path.find(".mp4"))
    {
      std::cout << "Serving Video File: " << path.c_str() << std::endl;
      // http::buffer_body::value buffer;

/* ------------------------------------------------------------------------ */ 
    // Set up the response. We use the buffer_body type,
    // allowing serialization to use manually provided buffers.
    http::response<boost::beast::http::buffer_body> res;
...
public:
    ...
    void on_read( beast::error_code ec, std::size_t bytes_transferred)
    {
        ...
        // Will be received as template<typename Body, typename Allocator, typename Send>
...
private:
    ...
    on_accept(beast::error_code ec, tcp::socket socket)
    {
        ...
        else
        {
          ...
          // make_shared is allowed because def is: 
          // class session : public std::enable_shared_from_this<session>
